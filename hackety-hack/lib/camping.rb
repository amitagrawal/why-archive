...
respond_to?:urls;c=self/c;c="//"+@env.REQUEST_APP+c if c[/^\//];URI(c)end;def/p
...
a.shift if a[0]==:render;s=Page.build(:self=>self,:mixin=>Views){send(*a,&b)}
...
NotFound<R();def get p;r(404,"<h1>#{P}</h1><h2>#{p} not found</h2>")end end;class 
ServerError<R();def get k,m,e;r(500,"<h1>#{P}</h1><h2>#{k}.#{m}</h2><h3>#{e.class
} #{e.message.to_html}:</h3><ul>#{e.backtrace.map{|bt|"<li>#{bt.to_html}</li>"}}</ul>")
end end;self;end;class<<
...
unpack('H2'*$&.size)*'%').upcase}end;def un s;s.tr('+',' ').gsub(
...
end;module Models;autoload:Base,'camping/db';def Y;self;end;end;class H<Hash;def method_missing m,*a
if m.to_s=~/=$/;self[$`]=a[0];elsif a.empty?;self[m];else;raise NoMethodError,
"#{m}" end end;alias_method:u,:update;end end
